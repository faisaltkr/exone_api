...
@frappe.whitelist(allow_guest=False)
def get_items_with_tax_template(company_name=None):
    try:
        # Apply company filter if provided
        item_filters = {}
        if company_name:
            item_filters['company_name'] = company_name

        # Fetch item details with the company_name filter
        items = frappe.get_all(
            'Item',
            filters=item_filters,
            fields=['name', 'item_code', 'item_name', 'description', 'standard_rate', 
                    'stock_uom', 'is_stock_item', 'image', 'item_name_arabic', 'item_group', 'company_name']
        )
        ...
        # Fetch barcodes and organize by item with additional price data
        barcodes = frappe.get_all('Item Barcode', fields=['parent', 'barcode', 'barcode_type', 'uom'])
        barcode_dict = {}
        for barcode in barcodes:
            barcode_dict.setdefault(barcode['parent'], []).append({
